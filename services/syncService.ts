...
const apiRequest = async (endpoint: string, method: string, body?: any, retries = 3, backoff = 300) => {
  try {
    ...
    // Clean token: Remove quotes and ensure it's not the string "undefined" or "null"
    const cleanToken = (token && token !== 'undefined' && token !== 'null') ? token.replace(/"/g, '') : null;
    
    if (cleanToken) {
        headers['Authorization'] = `Bearer ${cleanToken}`;
    } else {
        console.warn(`[SYNC] Outgoing ${method} to ${endpoint} with no token.`);
    }
...
const fromDbUser = (u: any): User => ({ id: u.id, org_id: u.org_id, name: u.name, email: u.email, role: u.role, status: u.status, avatarUrl: u.avatar_url, allowedProjectIds: safeParse(u.allowed_project_ids, []) });
...
const fromDbEnclosure = (e: any): Enclosure => ({ 
  ...
  boundary: safeParse(e.boundary, []), 
  individualIds: safeParse(e.individual_ids, []) 
});
...
const fromDbLoan = (l: any): BreedingLoan => ({ id: l.id, partnerOrgId: l.partner_org_id, proposerOrgId: l.proposer_org_id, role: l.role, start_date: l.start_date, end_date: l.end_date, status: l.status, individualIds: safeParse(l.individual_ids, []), terms: l.terms, notification_recipient_id: l.notification_recipient_id, changeRequest: safeParse(l.change_request, null) });
const fromDbPartnership = (p: any): Partnership => ({ id: p.id, orgId1: p.org_id_1, orgId2: p.org_id_2, status: p.status, established_date: p.established_date });
...
// Fixed dashboard_block and enable_enclosures to use correct camelCase properties from the Organization interface (line 162)
...
export const mapUserToDb = (u: User) => ({ id: u.id, org_id: u.org_id, name: u.name, email: u.email, role: u.role, status: u.status, password: u.password || null, avatar_url: u.avatarUrl || null, allowed_project_ids: u.allowedProjectIds || [] });
...
export const mapIndToDb = (i: Individual) => ({ 
  id: i.id, project_id: i.projectId, species_id: i.speciesId, enclosure_id: i.enclosureId || null, studbook_id: i.studbook_id, name: i.name, sex: i.sex, birth_date: i.birth_date || null, weight_kg: sanitizeNum(i.weightKg), sire_id: i.sireId || null, dam_id: i.damId || null, image_url: i.imageUrl || null, dna_sequence: i.dna_sequence || null, notes: i.notes || null, source: i.source || null, source_details: i.source_details || null, latitude: i.latitude ?? null, longitude: i.longitude ?? null, is_deceased: i.isDeceased ?? false, death_date: i.death_date || null, 
  // Fix: Changed i.loan_status to i.loanStatus to resolve property error
  loan_status: i.loanStatus || null, transferred_to_org_id: i.transferredToOrgId || null, transfer_date: i.transfer_date || null, transfer_note: i.transfer_note || null, weight_history: i.weightHistory || [], growth_history: i.growth_history || [], health_history: i.health_history || [] 
});
...
export const mapPartnershipToDb = (p: Partnership) => ({ id: p.id, org_id_1: p.orgId1, org_id_2: p.orgId2, status: p.status, established_date: p.established_date });

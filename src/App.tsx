...
// Added explicit imports for Component and ErrorInfo to ensure this.state and this.props are recognized
        ...
        <div className="p-6 flex flex-col gap-4 flex-shrink-0">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-2 text-emerald-700">
              {logoUrl ? <img src={logoUrl} alt="Logo" className="h-8 w-auto object-contain" /> : <PawPrint size={32} />}
  ...
  const performSync = async () => {
     ...
     try {
        ...
        if (result.success && result.data) {
           ...
           if (localUsers.length > 0 && (data.users || []).length === 0) {
           ...
           if (!getOrg().id && (result.message.includes('permission denied') || result.message.includes('42P01'))) setShowDbSetup(true);
        }
  ...
  return (
    <LanguageContext.Provider value={{ language: currentLangCode, setLanguage: setCurrentLangCode, t, refreshTranslations, availableLanguages: languages }}>
      <HashRouter>
        ...
        {showDbSetup && <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm"><div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]"><div className="p-6 border-b border-red-100 bg-red-50 flex justify-between items-center rounded-t-xl"><div className="flex items-center gap-3 text-red-800"><Database size={24} /><h3 className="text-xl font-bold">Database Setup Required</h3></div><button onClick={() => setShowDbSetup(false)} className="text-red-600 hover:text-red-800 bg-white/50 p-1 rounded-full"><X size={24}/></button></div><div className="p-6 overflow-y-auto space-y-4"><div className="bg-yellow-50 border-l-4 border-yellow-400 p-4"><p className="text-sm text-yellow-800 font-bold">Sync Error: {syncError || "Tables missing."}</p><p className="text-xs text-yellow-700 mt-1">Initialize your Supabase database using the SQL Editor.</p></div><h4 className="font-bold text-slate-900">How to Fix:</h4><ol className="list-decimal list-inside text-sm text-slate-700 space-y-2"><li>Open Supabase SQL Editor.</li><li>Paste the code below.</li><li>Click Run.</li></ol><div className="relative"><textarea readOnly className="w-full h-64 p-4 bg-slate-900 text-green-400 font-mono text-xs rounded-lg border border-slate-700 focus:outline-none" value={SUPABASE_SCHEMA_SQL} /><button onClick={() => { navigator.clipboard.writeText(SUPABASE_SCHEMA_SQL); showToast("SQL copied!", "success"); }} className="absolute top-4 right-4 bg-white/10 hover:bg-white/20 text-white p-2 rounded-lg backdrop-blur transition-colors" title="Copy SQL"><Copy size={16} /></button></div></div><div className="p-4 border-t border-slate-100 flex justify-end bg-slate-50 rounded-b-xl"><button onClick={() => { setShowDbSetup(false); performSync(); }} className="px-6 py-2 bg-emerald-600 text-white rounded-lg font-bold hover:bg-emerald-700 shadow-sm flex items-center gap-2"><RefreshCw size={18} /> Retry Sync</button></div></div></div>}
      </HashRouter>

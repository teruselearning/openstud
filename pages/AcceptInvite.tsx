...
const AcceptInvite: React.FC<AcceptInviteProps> = ({ onAccepted }) => {
  ...
  useEffect(() => {
    if (!token) { setStatus('error'); setError('No invitation token found in the link.'); return; }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
        <div className="flex flex-col items-center mb-8">
          ...
          {inviteData && (
            <p className="text-slate-500 text-sm mt-1 text-center">
              You've been invited to join <strong className="text-slate-800">{inviteData.orgName}</strong>
            </p>
          )}
        </div>
        ...
        {status === 'done' && (
          <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl p-4 flex items-center gap-3">
            ...
            <p className="text-sm font-bold">Account activated! Redirecting...</p>
          </div>
        )}
        ...
        {(status === 'ready' || status === 'submitting') && inviteData && (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-slate-50 rounded-xl p-4 text-sm text-slate-600 space-y-1 border border-slate-100">
              <p><span className="font-semibold text-slate-700">Name:</span> {inviteData.name}</p>
              <p><span className="font-semibold text-slate-700">Email:</span> {inviteData.email}</p>
            </div>
            <div>

...
const SuperAdmin: React.FC = () => {
  ...
  const uniqueGlobalSpecies = useMemo(() => {
    const query = searchQuery.toLowerCase().trim();
    // Unique by scientific name
  ...
  const holdersOfSpecies = useMemo(() => {
     ...
     // Find all organizations that have projects containing this species (by scientific name)
     const matchingSpeciesAcrossNetwork = allSpecies.filter(s => s.scientificName === selectedSpeciesForHolders.scientificName);
     const projectIdsHolding = matchingSpeciesAcrossNetwork.map(s => s.projectId);
     
     const results: { org: Organization | ExternalPartner, count: string, individualCount: number }[] = [];
     
     allOrganizations.forEach(org => {
        const orgProjects = allProjects.filter(p => (p.orgId || (p as any).org_id) === org.id);
        const orgProjectIds = orgProjects.map(p => p.id);
        const holdsInProjectIds = orgProjectIds.filter(id => projectIdsHolding.includes(id));
        
        if (holdsInProjectIds.length > 0) {
           // Calculate exact count for this organization
           const speciesInstances = matchingSpeciesAcrossNetwork.filter(s => holdsInProjectIds.includes(s.projectId));
           const speciesInstanceIds = speciesInstances.map(s => s.id);
           const orgInds = allIndividuals.filter(i => speciesInstanceIds.includes(i.speciesId) && !i.isDeceased);
           
           if (orgInds.length > 0) {
              const m = orgInds.filter(i => i.sex === Sex.MALE).length;
              const f = orgInds.filter(i => i.sex === Sex.FEMALE).length;
              const u = orgInds.filter(i => i.sex === Sex.UNKNOWN || !i.sex).length;
              results.push({
                 org,
                 count: `${m}.${f}.${u}`,
                 individualCount: orgInds.length
              });
           }
        }
     });
     ...
     return results;
  }, [selectedSpeciesForHolders, allSpecies, allProjects, allIndividuals, allOrganizations]);
  ...
  const handleAutoFillTranslations = async () => {
    ...
    if (Object.keys(missingKeysDict).length === 0) {
    ...
    try {
      ...
      if (translatedItems && Array.isArray(translatedItems)) {
        setEditingLang(prev => {
          ...
          translatedItems.forEach(item => { if (missingKeysDict[item.k]) mergedTranslations[item.k] = item.v; });
          return { ...prev, translations: mergedTranslations };
        });
  ...
  return (
    <div className="space-y-8 pb-12">
         ...
         <div className="space-y-6 animate-in fade-in">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
               <div className="flex bg-slate-100 p-1 rounded-xl">
                  <button onClick={() => setOverviewSubTab('organizations')} className={`px-4 py-1.5 text-sm font-bold rounded-lg transition-all ${overviewSubTab === 'organizations' ? 'bg-white shadow text-slate-900' : 'text-slate-500 hover:text-slate-900'}`}>Organizations</button>
                  <button onClick={() => setOverviewSubTab('species_browser')} className={`px-4 py-1.5 text-sm font-bold rounded-lg transition-all ${overviewSubTab === 'species_browser' ? 'bg-white shadow text-purple-700' : 'text-slate-500 hover:text-slate-900'}`}>Global Species Browser</button>
               </div>
               ...
               {overviewSubTab === 'species_browser' && (
                  <div className="relative w-full sm:w-72">
                     ...
                     <input 
                        type="text" 
                        placeholder="Search global species..." 
                        className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-purple-500 bg-white shadow-sm text-sm"
                        value={searchQuery}
            ...
            {overviewSubTab === 'organizations' ? (
                                                      ...
                                                      {hasSpeciesData ? (
                                                         <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                            {orgSpecies.map(sp => {
                                                               ...
                                                               const u = pop.filter(i => i.sex === 'Unknown' || !i.sex).length;
                                                               const counts = `${m}.${f}.${u}`;
               ...
               <div className="flex-1 overflow-y-auto bg-slate-50/50 rounded-2xl border border-slate-200 p-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                     {uniqueGlobalSpecies.map(s => (
                        <div key={s.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl transition-all group flex flex-col h-full cursor-pointer" onClick={() => setSelectedSpeciesForHolders(s)}>
                           ...
                           <div className="p-4 flex-1 flex flex-col">
                              <h3 className="font-bold text-slate-900 group-hover:text-purple-700 transition-colors leading-tight">{s.commonName}</h3>
                              ...
                              <div className="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
                                 <div className="flex items-center gap-1.5 text-purple-600 font-bold text-xs">
                                    View Organization Holders <ChevronRight size={14}/>
                                 </div>
                              </div>
                           </div>
                        </div>
                  ...
                  {uniqueGlobalSpecies.length === 0 && (
                     <div className="flex flex-col items-center justify-center py-20 text-slate-400 opacity-50">
                        ...
                        <p className="text-lg font-bold">No global species found matching your search.</p>
                     </div>
                  )}
               </div>
            )}
      ...
      {/* Holder Modal for Global Species Search */}
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in duration-200">
               <div className="p-6 border-b border-slate-100 flex justify-between items-start bg-slate-50">
                  <div className="flex items-center gap-4">
                     <div className="w-14 h-14 rounded-xl bg-purple-100 text-purple-600 flex items-center justify-center overflow-hidden border border-purple-200">
                        {selectedSpeciesForHolders.imageUrl ? <img src={selectedSpeciesForHolders.imageUrl} className="w-full h-full object-cover" /> : <Dna size={28}/>}
                     </div>
                     <div>
                  ...
                  <div className="grid gap-3">
                     {holdersOfSpecies.length === 0 ? (
                        <div className="py-12 text-center text-slate-400 italic bg-slate-50 rounded-xl border border-dashed border-slate-200">
                           No organization records for this species found.
                        </div>
                     ) : (
                        holdersOfSpecies.map(({ org, count, individualCount }) => (
                           <div key={org.id} className="bg-white p-4 rounded-xl border border-slate-200 flex items-center justify-between shadow-sm hover:border-purple-200 transition-colors">
                              <div className="flex items-center gap-3">
                                 ...
                                 <div>
                                    <h4 className="font-bold text-slate-900 flex items-center gap-2">
                                       {org.name}
                                       {org.id === myOrg?.id && <span className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded uppercase">Host</span>}
                                    </h4>
                              ...
                              <div className="flex items-center gap-4">
                                 <div className="text-right">
                                    <p className="text-[9px] font-bold text-slate-400 uppercase mb-1">Network Pop (M.F.U)</p>
                                    <p className="text-sm font-mono font-bold text-slate-700">{count}</p>
                                 </div>
                                 <button 
                                    onClick={() => {
                                       switchOrganization(org.id, org) && window.location.reload();
                                    }}
                                    className="p-2 rounded-lg border border-slate-200 text-slate-600 hover:text-purple-600 hover:bg-purple-50 hover:border-purple-200 shadow-sm transition-all"
               ...
               <div className="p-4 bg-slate-50 border-t border-slate-200 text-center">
                  <p className="text-[10px] text-slate-400 font-medium">As a Super Admin, you are seeing exact population data regardless of organization privacy settings.</p>
               </div>
            </div>
         </div>
      )}
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-fit min-h-[600px]">
               ...
               <div className="flex gap-1 mb-4 bg-slate-100 p-1 rounded-xl">
                  {['registration', 'mfa', 'invite', 'notification'].map(tKey => (
                     <button key={tKey} onClick={() => handleTemplateChange(tKey)} className={`flex-1 py-1 text-[10px] font-extrabold rounded-lg uppercase tracking-wider transition-all ${selectedTemplate === tKey ? 'bg-white shadow text-emerald-700' : 'text-slate-500 hover:text-slate-800'}`}>{tKey}</button>
                  ))}
               </div>
            ...
            <div className="p-6 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50/50">
               ...
               <div className="flex gap-2">
                  <div className="relative group">
                     <input placeholder="en-GB" title="Language Code" className="border border-slate-300 px-3 py-1.5 rounded-xl text-sm w-20 bg-white outline-none focus:ring-2 focus:ring-purple-500" value={newLangCode} onChange={e => setNewLangCode(e.target.value)} />
                  </div>
                  ...
                  <button onClick={async () => { if(!newLangCode || !newLangName) return; const updated = [...languages, { code: newLangCode, name: newLangName, translations: { ...BASE_TRANSLATIONS }, isDefault: false }]; setLanguages(updated); await saveLanguages(updated); refreshTranslations(); setNewLangCode(''); setNewLangName(''); }} className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-1.5 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-2"><Plus size={14}/> {t('add')}</button>
               </div>
            </div>
            <div className="flex flex-1 overflow-hidden">
               <div className="w-64 border-r border-slate-200 bg-slate-50 overflow-y-auto p-4 space-y-1">
                  {languages.map(l => (
                     <button key={l.code} onClick={() => { setEditingLang(l); setAiFillSuccess(false); }} className={`w-full text-left p-3 rounded-lg flex justify-between items-center transition-all ${editingLang?.code === l.code ? 'bg-white shadow text-purple-700 font-extrabold ring-1 ring-purple-100' : 'hover:bg-white/60 text-slate-600 font-medium text-sm'}`}>
                        <span>{l.name}</span>
               ...
               <div className="flex-1 overflow-y-auto p-0 bg-white flex flex-col">
                  {editingLang ? (
                     <div className="flex flex-col h-full animate-in fade-in duration-200">
                        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white sticky top-0 z-10 shadow-sm">
                           <div className="flex-1 flex items-center gap-4 w-full">
                              <h4 className="font-extrabold text-xl text-slate-800 tracking-tight whitespace-nowrap">{editingLang.name}</h4>
                              <div className="relative flex-1 max-w-sm">
                                 <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                                 <input className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-purple-500 outline-none transition-all" placeholder="Filter translation keys..." value={translationSearch} onChange={e => setTranslationSearch(e.target.value)} />
                                 {translationSearch && <button onClick={() => setTranslationSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"><X size={14}/></button>}
                              </div>
                           </div>
                           <div className="flex items-center gap-3">
                             {aiFillSuccess && <div className="flex items-center gap-1.5 text-emerald-600 font-bold text-xs animate-in slide-in-from-right-2"><Check size={14} /> {t('autofill')} Success</div>}
                             <button onClick={handleAutoFillTranslations} disabled={isAutoFilling} className="bg-indigo-50 text-indigo-700 hover:bg-indigo-100 px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all disabled:opacity-50">{isAutoFilling ? <Loader2 className="animate-spin" size={14}/> : <Wand2 size={14}/>} {t('autofill')}</button>
                           </div>
                        </div>
                        <div className="p-8 space-y-6">
                           {filteredTranslationKeys.length === 0 ? (<div className="text-center py-20 text-slate-400"><Search size={48} className="mx-auto mb-3 opacity-20"/><p>No translation keys match your filter.</p></div>) : filteredTranslationKeys.map(key => (
                              <div key={key} className="space-y-1 group"><div className="flex items-center justify-between"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">{key}</label><span className="text-[9px] font-bold text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity">SOURCE: {(BASE_TRANSLATIONS as any)[key]}</span></div>{key.toLowerCase().includes('body') || key.toLowerCase().includes('html') ? (<div className="rounded-xl border border-slate-200 overflow-hidden shadow-sm"><RichTextEditor value={String(editingLang.translations[key] || '')} onChange={v => handleUpdateTranslation(key, v)} height="200px" /></div>) : (<input className="w-full border border-slate-200 px-4 py-2 rounded-lg text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-purple-500 outline-none transition-all" value={String(editingLang.translations[key] || '')} onChange={e => handleUpdateTranslation(key, e.target.value)} />)}</div>
                           ))}
                        </div>
                     </div>
                  ) : (<div className="flex flex-col items-center justify-center h-full text-slate-300 space-y-4 opacity-50"><Globe size={80} strokeWidth={1} /><p className="font-extrabold text-lg">Select a language to translate</p></div>)}
               </div>
            </div>
            {editingLang && (<div className="p-4 border-t border-slate-200 bg-slate-50 flex justify-end gap-3"><button onClick={() => setEditingLang(null)} className="px-6 py-2 text-slate-600 font-bold hover:bg-slate-200 rounded-xl transition-all text-sm">{t('cancel')}</button><button onClick={handleSaveTranslations} className="bg-purple-600 hover:bg-purple-700 text-white px-10 py-2 rounded-xl font-bold shadow-lg shadow-purple-100 flex items-center gap-2 active:scale-95 transition-all text-sm">{isSavingLang ? <Loader2 size={16} className="animate-spin"/> : <Save size={16}/>} {t('save')}</button></div>)}
         </div>
      )}
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full flex flex-col overflow-hidden animate-in zoom-in duration-200">
               <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                     ...
                     <div><h3 className="text-xl font-extrabold text-slate-900">{t('createOrgBtn')}</h3><p className="text-xs text-slate-500">Create an org and its primary admin user.</p></div>
                  </div>
               ...
               <form onSubmit={handleCreateOrgSubmit} className="p-8 space-y-6">
                  <div className="space-y-4">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase block tracking-wider">{t('orgName')}</label><input className="w-full px-4 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-purple-500 bg-white text-slate-900 text-sm font-bold" placeholder="e.g. Island Sanctuary" value={String(newOrgData.orgName || '')} onChange={e => setNewOrgData({...newOrgData, orgName: e.target.value})} required /></div>
                        <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase block tracking-wider">Org Focus</label><select className="w-full px-4 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-purple-500 bg-white text-slate-900 text-sm font-bold" value={newOrgData.focus || 'Animals'} onChange={e => setNewOrgData({...newOrgData, focus: e.target.value as OrganizationFocus})}><option value="Animals">{t('animal')}</option><option value="Plants">{t('plant')}</option></select></div>
                        <div className="col-span-1 md:col-span-2 space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase block tracking-wider">{t('location')}</label><div className="relative"><MapPin size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-purple-500 bg-white text-slate-900 text-sm" placeholder="e.g. Victoria, Seychelles" value={String(newOrgData.location || '')} onChange={e => setNewOrgData({...newOrgData, location: e.target.value})} required /></div></div>
                     </div>
                  </div>
                  <div className="space-y-4 pt-6 border-t border-slate-50">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase block tracking-wider">{t('name')}</label><input className="w-full px-4 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-purple-500 bg-white text-slate-900 text-sm" placeholder="John Smith" value={String(newOrgData.adminName || '')} onChange={e => setNewOrgData({...newOrgData, adminName: e.target.value})} required /></div>
                        <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase block tracking-wider">{t('emailAddr')}</label><input type="email" className="w-full px-4 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-purple-500 bg-white text-slate-900 text-sm" placeholder="john@island-sanctuary.org" value={String(newOrgData.adminEmail || '')} onChange={e => setNewOrgData({...newOrgData, adminEmail: e.target.value})} required /></div>
                     </div>
                  </div>
                  <div className="flex gap-3 pt-6"><button type="button" onClick={() => setShowCreateOrg(false)} disabled={isCreatingOrg} className="flex-1 px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold transition-colors disabled:opacity-50">{t('cancel')}</button><button type="submit" disabled={isCreatingOrg} className="flex-1 px-4 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-purple-200 flex items-center justify-center gap-2">{isCreatingOrg ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />} Confirm & Send Invite</button></div>
               </form>
            </div>
         </div>
      )}
      ...
      {orgToDelete && (
          <div className="fixed inset-0 bg-black/60 z-[200] flex items-center justify-center p-4 backdrop-blur-sm">
             <div className="bg-white rounded-xl shadow-2xl max-md w-full p-8 text-center animate-in zoom-in duration-200">
                <div className="w-20 h-20 bg-red-100 text-red-600 rounded-full flex items-center justify-center mx-auto mb-6"><AlertTriangle size={40}/></div>
                ...
                <div className="text-slate-500 mb-8 leading-relaxed text-sm text-left"><p className="mb-2">This action is <span className="text-red-600 font-bold">permanent</span> and will delete all:</p><ul className="list-disc px-8 space-y-1"><li>Users and accounts</li><li>All projects and species data</li><li>Individual records and breeding logs</li><li>Genetic data and photos</li></ul></div>
                <div className="flex gap-3"><button onClick={() => setOrgToDelete(null)} disabled={isDeleting} className="flex-1 px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold transition-colors disabled:opacity-50">{t('cancel')}</button><button onClick={handleDeleteOrg} disabled={isDeleting} className="flex-1 px-4 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-red-200 flex items-center justify-center gap-2 disabled:opacity-50">{isDeleting ? <Loader2 size={18} className="animate-spin" /> : <Trash2 size={18} />} {t('delete')}</button></div>
             </div>
          </div>
       )}
    </div>
